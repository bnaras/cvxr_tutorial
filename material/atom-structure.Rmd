...
$$
TODO: Describe S4 object constructor and inheritance from Atom superclass.
```{r, eval=FALSE}
setClass("QuadOverLin", representation(x = "ConstValORExpr", y = "ConstValORExpr"), contains = "Atom")
```
For convenience, we also define a constructor
```{r, eval=FALSE}
quad_over_lin <- function(x, y) { new("QuadOverLin", x = x, y = y) }
```

We must pass `x` and `y` to the `Atom` superclass so they can be saved in its `args` slot. This gives inherited methods a consistent way of accessing the arguments of any atom. To achieve this, we override the default initialization with
```{r, eval=FALSE}
...
```
TODO: Explain what is happening in each line.

## Atom Properties
Now that we have created the S4 class, it's time to define methods that characterize the atom's mathematical properties.
...
```
The input `object` is a `QuadOverLin` atom. We access its arguments (which are `Expression` objects) via its `args` slot. Given our initialization, `object@args[[1]]` contains the expression for $x$ and `object@args[[2]]` the one for $y$. We invoke `is_scalar`, a method in the `Expression` class, on the latter to check if $y$ is indeed scalar. If this check passes, then a call is made up the stack to `validate_args` in its superclass, `Atom`.
