...
$$
TODO: S4 object constructor and initialize.
...
Both validation and domain methods may be dropped if an atom's arguments span the reals. However, the `to_numeric` method is always required. It takes as input an atom and a list `values` containing the numeric values of the arguments (given in the same order as `object@args`), then evaluates the atom at these values. For our `QuadOverLin` atom, this method is simply
```{r, eval=FALSE}
...
The methods we have just described are the bare minimum required to define an atom. Many additional methods may be implemented, which handle complex numbers, support new solver classes, or provide further functional characterization. For instance, we furnish a method that computes the gradient of the quadratic-over-linear atom,
$$
