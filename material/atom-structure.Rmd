...
```
The `QuadOverLin` class takes as input two arguments, `x` and `y`, which must be R numeric constants (`numeric`, `matrix`, etc) or `Expression` objects. These input types are encapsulated in the "ConstaValORExpr" class union. Since `Expression`s themselves may contain constants, variables, and other atoms, this allows us to use the quadratic-over-linear function in nested compositions. TODO: `QuadOverLin` is an atom and thus inherits from the `Atom` superclass... what slots/methods?
...
```
The first two lines save `x` and `y` to their respective slots, while the third calls the superclass initializer with `atom_args` equal to the list of arguments.
```{r, eval=FALSE}
...
```
The input `object` is a `QuadOverLin` atom. We access its arguments via its inherited `args` slot. Given our initialization, `object@args[[1]]` contains the argument for $x$ and `object@args[[2]]` the one for $y$. We invoke `is_scalar`, a method in the `Expression` class, on the latter to check if $y$ is indeed scalar. If this check passes, then a call is made up the stack to `validate_args` in its `Atom` superclass.

Notice that we did not check if $y > 0$ during validation. This is because we generally do not know the value of an argument during the construction of a quadratic-over-linear expression - $y$ may contain a `Variable` that we are solving for in our problem, and we cannot define the problem if validation fails. It is the user's responsibility to include domain constraints when constructing a `Problem`.

To facilitate this, for each atom, we define a method that returns a list of `Constraint` objects delineating that atom's domain. For the quadratic-over-linear atom, we need only constrain its second argument $y$ to be positive. Strict inequalities are not supported in `CVXR`, so we impose a weak inequality $y \geq 0$ as seen below.
```{r, eval=FALSE}
...
```
Note the difference between `object@args` and `values`. The former is a list of `Expression`s, which represent a composition of constants, variables, and other nested atoms. The latter is a list of R objects such as `numeric` and `matrix`, denoting the values of the corresponding expressions.
...
TODO: Ready to see how CVXR uses atom to analyze, reduce, and solve problem. Suppose we are given $A \in \mathbf{R}^{m \times n}$ and $b \in \mathbf{R}^m$, and would like to solve
$$
  \begin{array}{ll}
    \mbox{minimize} & f(x,y) \\
...
$$
  ...
  \end{array}
$$
This SOCP canonicalization may then be passed into a cone solver to obtain the optimal $t^* = f(x^*,y^*)$.

TODO: How does CVXR automatically canonicalize an atom?
