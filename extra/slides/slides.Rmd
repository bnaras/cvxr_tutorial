...
Even before attempting a solution, one has to verify a problem is convex. A few possible approaches are:
...
[@fu:naras:boyd:2019] that is due to appear in the [Journal of Statistical
...
- Log-barrier: $-\sum_{i=1}^m\log(−f_i(x))$ (on $\{x \in \mathbf{R}^n : f_i(x) < 0\}$, $f_i$ convex)
...
1. Modify the objective to perform least absolute deviation (LAD) regression
...
- `CVXR`'s syntax is simple and mathematically intuitive. 
