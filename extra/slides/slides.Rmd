...
- [`CVXR`](https://cran.r-project.org/package=CVXR) version 0.99-6,
...
When $f_i(x) \leq 0$ for all $i$ constrain $x$ to lie in a convex cone $C$, the problem is called a cone program. Examples of $C$ are the positive orthant $\mathbf{R}^n$, the set of positive semidefinite matrices $\mathbf{S}_+^n$, and the second-order cone $\{(x,t) \in \mathbf{R}^n \times \mathbf{R}: \|x\| \leq t\}$.
...
`CVXR`?  _Hint:_ The `result$getValue()` evalutes expressions in the
problem context.
...
The `CVXR::pos` atom evaluates $x_+$ elementwise on the input
expression.
...
- Introduce `CVXR::Semidef` and `CVXR::log_det`
...
positive semidefinite cone. We use `CVXR` functions 
for the log-determinant and trace in the objective. The expression `matrix_trace(S %*%
...
## Markovwitz Portfolio Problem

The Markovwitz portfolio problem [@Markowitz:1952; @Roy:1952;
...
`atom_args` equal to the list of arguments. For explanatory purposes,
...
`atom_args`, we know `object@args[[1]]` contains the argument for $x$
...
The `to_numeric` method is always required. It takes as input an atom and a list `values` containing the numeric values of the its arguments (given in the same order as in `atom_args`), then evaluates the atom at these values. For the QoL function, this method is simply
...
`Expression` class method `is_nonneg` (resp. `is_nonpos`) to determine
...
define an atom. Additional methods may be implemented that handle
complex numbers, support new solver classes, or provide further
...
Furthermore, we allow CVXR to solve certain problems involving QoL
functions as a quadratic program (QP). Before calling a QP solver,
CVXR must verify that an objective function is quadratic. It is easy
to see that $f(x,y)$ is quadratic if its first argument is an affine
expression and its second argument is constant. Therefore, we define
the method
...
where `is_affine` and `is_constant` are given in the `Expression`
class. A similar method exists for checking if $f(x,y)$ is the
quadratic of a piecewise affine function.
...
through a series of calls to individual atom canonicalizers, defined
in the reductions framework. To implement an atom, we must explicitly
derive its canonicalizer for a particular class of problems (e.g.,
conic programs) supported by the desired solver. Below we provide a
derivation based on the graph implementation.
...
In `CVXR`, this canonicalizer function is defined in dcp2cone.R, since
it is used to reduce a DCP problem to a cone program.
...
It takes as input a `QuadOverLin` expression `expr` and a list of
arguments `args`, which specify the input values $(x,y)$. The first
two lines of the function extract $x$ and $y$ from `args`, and the
third line constructs the `Variable` $t \in \mathbf{R}$. The fourth
line forms the constraint $\left(\begin{pmatrix} y-t \\ 2x
\end{pmatrix}, y+t\right) \in \mathcal{K}$ with a call to the
constructor `SOC`. If $u \in \mathbf{R}^n$, then `SOC(t,u)` enforces
$\|u\|_2 \leq t$. Finally, the canonicalizer returns a list containing
$t$, the epigraph variable, and a list of `Constraint`s from the graph
implementation - in this case, the single second-order cone
constraint.
...
expression tree. It then constructs a chain of *reductions*, which
reduces the problem as modeled to a form compatible with the desired
solver.

In our example, one of these reductions is a transformation into a
...
call to `DCP2Cone.quad_over_lin_canon`. Then, in a final reduction,
the relevant matrices are formed and passed to the selected conic
solver. After the solver returns $(x^*,y^*,t^*)$, this chain of
reductions is inverted to obtain the optimal point $(x^*,y^*)$ and
