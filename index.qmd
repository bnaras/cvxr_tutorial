...
session**, with `quarto render` (or, in RStudio, the **Render**
button, available any time a `.qmd` file is open): this caches
every chunk's results via Quarto's *freeze*, so nothing has to recompute live during the
tutorial, and it confirms your whole setup --- packages and solvers
--- in one shot.
